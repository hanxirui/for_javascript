...
var BackboardService = {
	...
	insertBg: function(bgInfo, $callback) {
		...
		commander.saveBySqlInsert(sql, param).then(function(recordset){
	...
	deleteBpData : function(backGroudId,$callback){
		...
		commander.delBySql(sql, data).then(function(recordset){
	...
	upDataBackPlane : function(bgInfo,$callback){
		...
		commander.getBySql(sql, bgInfo).then(function(recordset){
	    $callback(recordset.isError);
		}).fail(function(err){
    	console.log(err.toString());
		})
