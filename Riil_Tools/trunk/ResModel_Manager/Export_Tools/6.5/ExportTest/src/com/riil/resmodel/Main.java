...
import com.riil.resmodel.services.impl.ResModelService;
    ...
    public class Main {  
		...
		private static void init(String[] args) {
        	...
        	InputStream t_resourceAsStream = Main.class.getClassLoader().getResourceAsStream("conf.properties");
        	try {
				props.load(t_resourceAsStream);
			} catch (Exception e) {
				e.printStackTrace();
			} 
//        	String t_exportUrl = props.getProperty("export_url");
        	String t_exportUrl = args[0];
        	export_model_url = t_exportUrl+"model/";
        	export_policy_url = t_exportUrl+"policy/";
        	export_resType_url = t_exportUrl+"resType/";
        	export_metricBase_url = t_exportUrl+"metric/"; 
        	export_dict_url =t_exportUrl+"dict/"; 
             BasicDataSource t_dataSource = (BasicDataSource) context.getBean("dataSource");
             
             t_dataSource.setUrl(args[3]);
             t_dataSource.setUsername(args[1]);
             t_dataSource.setPassword(args[2]);
