...
var BackboardService = {
    ...
    insertBg: function (bgInfo, $callback) {
        ...
        commander.saveBySqlInsert(sql, param, {
            userId: userId,
            info: logContent
    ...
    queryBpData: function ($callback) {
        var sql = "select c_id,c_backplane_name,c_project_name,c_ventor,c_equipment_type,DATE_FORMAT(c_apply_date,'%Y-%m-%e') as c_apply_date,c_operator_id,c_desc from t_ml_back_plane";
    ...
    upDataBackPlane: function (bgInfo, $callback) {
        var userId = bgInfo.operator;
        var logContent = "背板信息管理,修改操作:" + "c_id:" + bgInfo.bpId;
        ...
        commander.saveBySqlUpdate(sql, bgInfo, {
            userId: userId,
            info: logContent
        }).then(function (recordset) {
            $callback(recordset.isError);
