...
function SqlCommand() {
     ...
     */
    this.save = function (sqlKey, paramJson) {
        ...
        try {
            ...
            if (matchArray) {
                ...
                if ((sqlAction.toLowerCase() !== strInsert.toLowerCase()) && (sqlAction.toLowerCase() !== strUpdate.toLowerCase())) {
                    ...
                    DataSource.getInstance().ds.withSession(function (session) {
     ...
     */
    this.saveBySqlInsert = function (sqlStr, sqlParam) {
        var recordSet = new RecordSet();
        var getQ = q.defer();
     ...
     * @param {json}   paramJson     插入或者更新对应的参数json串
     */
    this.saveBySqlUpdate = function (sqlStr, sqlParam) {
        var recordSet = new RecordSet();
        var getQ = q.defer();
     ...
     * @param {json}   paramJson     插入或者更新对应的参数json串
     */
    this.del = function (sqlKey, paramJson) {
        var recordSet = new RecordSet();
        var getQ = q.defer();
     ...
     */
    this.delBySql = function(sqlStr,sqlParam) {
        var recordSet = new RecordSet();
        var getQ = q.defer();
