...
$(document).ready(function(){
	...
	$.ajax({
		url : ctx+"/resourceModelTreeController/getResTypeTree",
		data : "",
		type : "post",
		dataType : "json",
...
function submitModelInfo(){
	...
	if(checkModelId()){
		$.ajax({
			type:'post',
			url:ctx+"/resourceTypeCotroller/addResourceModelInfo",
			data:modelInfo,
			dataType:'text',
			...
			success:function(data){
				if(data!='0'){
					...
					iframedom.src = ctx + "/resourceTypeCotroller/resourceModelDetail?id="
							+ data;
 ...
 */
function getIndAdd() {
	$.post(ctx + "/resourceTypeCotroller/getModelIndicatorAdd", function(data) {
		myAlert({
			title : '添加指标信息',
			msg : data,
			type : 'alert',
